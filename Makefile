...
build_lambda_package: build_swift gather_dependencies
...
run_test:
	echo "Running executable on Linux"
	docker run --rm -v "$(shell pwd)/swiftcommand:/src" -w /src ubuntu /bin/bash -c 'LD_LIBRARY_PATH=/src/LinuxLibraries /src/.build/release/swiftcommand < /src/TestData/testInput.json'
...
test:
	...
	docker run \
               ...
               ibmcom/kitura-ubuntu \
               swift build
	docker run --rm -v "$(shell pwd)/swiftcommand:/src" -w /src ibmcom/kitura-ubuntu swift test
